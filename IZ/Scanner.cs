...
public class Scanner {
	...
	static Scanner() {
		...
		for (int i = 115; i <= 118; ++i) start[i] = 1;
	...
	void CheckLiteral() {
		switch (t.val) {
			case "changeR": t.kind = 11; break;
			case "is": t.kind = 13; break;
			case "changeS": t.kind = 15; break;
			case "q": t.kind = 22; break;
			case "L": t.kind = 24; break;
			case "R": t.kind = 25; break;
			case "N": t.kind = 26; break;
			case "if": t.kind = 27; break;
			case "else": t.kind = 28; break;
			case "T": t.kind = 41; break;
			case "A": t.kind = 42; break;
			case "Q": t.kind = 43; break;
			case "P": t.kind = 44; break;
			case "new": t.kind = 45; break;
			case "as": t.kind = 46; break;
			case "TuringMachine": t.kind = 47; break;
			case "SetOfStates": t.kind = 48; break;
			case "RuleSet": t.kind = 49; break;
			case "Integer": t.kind = 50; break;
	...
	Token NextToken() {
		...
		switch (state) {
			...
			case 43:
				...
				else if (ch == 'Q') {AddCh(); goto case 20;}
				else if (ch == 'P') {AddCh(); goto case 21;}
				else {goto case 0;}
			case 44:
				recEnd = pos; recKind = 1;
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch >= 'a' && ch <= 'g' || ch >= 'i' && ch <= 'q' || ch >= 's' && ch <= 'z') {AddCh(); goto case 1;}
				else if (ch == 'h') {AddCh(); goto case 46;}
				else if (ch == 'r') {AddCh(); goto case 47;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 45:
				recEnd = pos; recKind = 1;
			...
			case 51:
				recEnd = pos; recKind = 1;
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch >= 'a' && ch <= 'c' || ch >= 'e' && ch <= 'z') {AddCh(); goto case 1;}
				else if (ch == 'd') {AddCh(); goto case 54;}
				else {t.kind = 1; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 52:
				recEnd = pos; recKind = 1;
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch >= 'a' && ch <= 'd' || ch >= 'f' && ch <= 'z') {AddCh(); goto case 1;}
